...
class RBM(object):
    ...
    def training(self, train, valid, user, epochs, batchsize, free_energy, verbose, filename):
        ...
        if not os.path.exists(const.DIR_RBM_MODELS):
            os.mkdir(const.DIR_RBM_MODELS)
        
        dir_filename = const.DIR_RBM_MODELS + filename
        
        if not os.path.exists(dir_filename):
            os.mkdir(dir_filename)
        
        np.save(dir_filename + '/w.npy', prv_w)
        np.save(dir_filename + '/vb.npy', prv_vb)
        np.save(dir_filename + '/hb.npy', prv_hb)
        
        if free_energy:
            print("Exporting free energy plot")
            self.export_free_energy_plot(dir_filename)
    ...
    def load_predict(self, filename, train, user):
        ...
        prv_w = np.load(const.DIR_RBM_MODELS + filename + '/w.npy')
        prv_vb = np.load(const.DIR_RBM_MODELS + filename + '/vb.npy')
        prv_hb = np.load(const.DIR_RBM_MODELS + filename + '/hb.npy')
    ...
    def calculate_scores(self, ratings, attractions, rec, user):
        ...
        for place in places_id:
            ...
            places_prices.append(
                attractions[attractions['attraction_id'] == place]['avg_price'].tolist()[0])
